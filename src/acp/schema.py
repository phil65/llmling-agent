...
#   timestamp: 2025-10-27T00:02:21+00:00
...
class CurrentModeUpdate(Schema):
    ...
    current_mode_id: Annotated[str, Field(description="The ID of the current mode")]
...
class AgentErrorMessage(Schema):
...
class ClientErrorMessage(Schema):
    """A message (request, response, or notification) with `"jsonrpc": "2.0"` specified as
    [required by JSON-RPC 2.0 Specification][1].
...
class AvailableCommandsUpdate(Schema):
    ...
    available_commands: Annotated[
        Sequence[AvailableCommand], Field(description="Commands the agent can execute")
    ]
...
class RequestPermissionRequest(Schema):
    ...
    session_id: Annotated[str, Field(description="The session ID for this request.")]
    tool_call: Annotated[
        ToolCall, Field(description="Details about the tool call requiring permission.")
    ]
