...
class BaseEventStream(ABC, Generic[RunRequestT, EventT, AgentDepsT]):
    ...
    async def encode_stream(
        self, stream: AsyncIterator[EventT], accept: str | None = None
    ) -> AsyncIterator[str]:
    ...
    async def handle_stream(
        self, stream: AsyncIterator[SourceEvent]
    ...
    async def handle_part_start(
        ...
        match part:
            case TextPart():
                async for e in self.handle_text_start(
                    part, follows_text=isinstance(previous_part, TextPart)
                ):
                    yield e
            case ThinkingPart():
    ...
    async def handle_part_end(
        ...
        match part:
            case TextPart():
                async for e in self.handle_text_end(
                    part, followed_by_text=isinstance(next_part, TextPart)
                ):
                    yield e
            case ThinkingPart():
                async for e in self.handle_thinking_end(
                    part, followed_by_thinking=isinstance(next_part, ThinkingPart)
                ):
                    yield e
            case ToolCallPart():
