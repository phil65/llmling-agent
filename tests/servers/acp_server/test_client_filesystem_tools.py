...
async def test_read_file_with_line_and_limit(fs_provider: FSSpecTools):
    ...
    tools = await fs_provider.get_tools()
    read_tool = next(tool for tool in tools if tool.name == "read_file")
    result = await read_tool.execute(agent_ctx=CTX, path="/home/large_file.txt", line=2, limit=2)
    ...
    fs_provider.fs._cat_file.assert_called_once_with("/home/large_file.txt")
...
async def test_write_text_file_json(fs_provider: FSSpecTools):
    ...
    write_tool = next(tool for tool in tools if tool.name == "write_file")
    result = await write_tool.execute(
        agent_ctx=CTX, path="/home/user/config.json", content=json_str
    )
    assert isinstance(result, dict)
...
async def test_read_empty_file(fs_provider: FSSpecTools):
    ...
    tools = await fs_provider.get_tools()
    read_tool = next(tool for tool in tools if tool.name == "read_file")
    result = await read_tool.execute(agent_ctx=CTX, path="/home/user/empty.txt")
...
async def test_write_empty_file(fs_provider: FSSpecTools):
    ...
    tools = await fs_provider.get_tools()
    write_tool = next(tool for tool in tools if tool.name == "write_file")
    result = await write_tool.execute(agent_ctx=CTX, path="/home/user/empty_output.txt", content="")
    assert isinstance(result, dict)
...
async def test_read_file_with_unicode(fs_provider: FSSpecTools):
    ...
    fs_provider.fs._cat_file = AsyncMock(return_value=content.encode("utf-8"))
    # Get read_file tool from provider
    tools = await fs_provider.get_tools()
    read_tool = next(tool for tool in tools if tool.name == "read_file")
...
async def test_write_file_with_unicode(fs_provider: FSSpecTools):
    ...
    mock_file.write = AsyncMock()
    fs_provider.fs.open_async = AsyncMock(return_value=mock_file)
    fs_provider.fs._info = AsyncMock(return_value={"size": len(content)})
    # Get write_text_file tool from provider
    tools = await fs_provider.get_tools()
    write_tool = next(tool for tool in tools if tool.name == "write_file")
    result = await write_tool.execute(agent_ctx=CTX, path="/home/output.txt", content=content)
    assert isinstance(result, dict)
    assert result["path"] == "/home/output.txt"
    # Verify unicode content was written correctly
    fs_provider.fs.open_async.assert_called_once_with("/home/output.txt", "wt")
...
async def test_file_operations_with_provider_session(fs_provider):
    ...
    write_tool = next(tool for tool in tools if tool.name == "write_file")
    await write_tool.execute(agent_ctx=CTX, path="/home/user/test.txt", content="test content")
    # Verify filesystem calls were made
